...
class Admin_UI
{
	 ...
	 */
	public $class_name = 'WC_Compare';

	public $plugin_prefix = WOOCP_PREFIX;
	 ...
	 */
	public $admin_pages = array( 'woo-compare-features', 'woo-compare-products', 'product_attributes' );

	public function __construct() {
		$this->google_api_key_option     = WOOCP_KEY . '_google_api_key';
		$this->google_map_api_key_option = WOOCP_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WOOCP_KEY . '_toggle_box_open';
		$this->version_transient         = WOOCP_KEY . '_licinfo';

		if ( defined( 'WOOCP_G_FONTS' ) ) {
			$this->is_load_google_fonts = (boolean) WOOCP_G_FONTS;
		}
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'woocommerce-compare-products' ),
				'image_url'    => WOOCP_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'woocommerce-compare-products' ),
				'left_text'    => __( 'WooCommerce Dynamic Gallery Premium', 'woocommerce-compare-products' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the WooCommerce Dynamic Gallery Premium version', 'woocommerce-compare-products' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'woocommerce-compare-products' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'woocommerce-compare-products' )
									. "\n\n" . '<a target="_blank" href="'.$this->pro_plugin_page_url.'" class="button-primary">' . __( 'Get Premium Features and Support', 'woocommerce-compare-products' ) . '</a>',
			);
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'woocommerce-compare-products' )
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'woocommerce-compare-products' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'woocommerce-compare-products' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'woocommerce-compare-products' ), $this->support_url );
			} else {
