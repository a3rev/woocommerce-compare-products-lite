...
add_filter( $wc_compare_admin_init->plugin_name . '_plugin_extension_boxes', array( 'WC_Compare_Hook_Filter', 'plugin_extension_box') );

// Add extra link on left of Deactivate link on Plugin manager page
add_action('plugin_action_links_' . WOOCP_NAME, array( 'WC_Compare_Hook_Filter', 'settings_plugin_links' ) );
...
add_action( 'wp_enqueue_scripts', array( 'WC_Compare_Hook_Filter', 'add_google_fonts'), 9 );
...
if ( ( ! isset( $woo_compare_product_page_settings['disable_product_page_compare'] ) || $woo_compare_product_page_settings['disable_product_page_compare'] != 1 ) && ( ! isset( $woo_compare_product_page_settings['disable_compare_featured_tab'] ) || $woo_compare_product_page_settings['disable_compare_featured_tab'] != 1 ) ) {
	if ( version_compare( $current_db_version, '2.0', '<' ) && null !== $current_db_version ) {
		add_action( 'woocommerce_product_tabs', array('WC_Compare_Hook_Filter', 'woocp_product_featured_tab'), $woo_compare_product_page_settings['auto_compare_featured_tab'] );
		add_action( 'woocommerce_product_tab_panels', array('WC_Compare_Hook_Filter', 'woocp_product_featured_panel'), $woo_compare_product_page_settings['auto_compare_featured_tab']);
	} else {
		// woo 2.0
...
if (in_array(basename($_SERVER['PHP_SELF']), array('post.php', 'page.php', 'page-new.php', 'post-new.php'))) {
	add_action('admin_footer', array('WC_Compare_MetaBox', 'admin_include_variation_compare_scripts'));
}
// For variations
add_action( 'woocommerce_product_after_variable_attributes', array('WC_Compare_MetaBox', 'variable_compare_meta_boxes'), 1, 3 );
add_action( 'woocommerce_save_product_variation', array('WC_Compare_MetaBox', 'save_product_variation'), 11, 2 );

if (in_array(basename($_SERVER['PHP_SELF']), array('post.php', 'page.php', 'page-new.php', 'post-new.php'))) {
	add_action('save_post', array('WC_Compare_MetaBox', 'save_compare_meta_boxes' ), 11 );
}
