...
-----------------------------------------------------------------------------------*/
class WC_Compare_Uploader extends WC_Compare_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $attachment_id = 0, $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $strip_methods = true ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'woocommerce-compare-products' ).'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'woocommerce-compare-products' );
...
global $wc_compare_uploader;
$wc_compare_uploader = new WC_Compare_Uploader();
